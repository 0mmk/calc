...
double str_list(char *str, vlist *v) {
	...
	for (; str[i]; i++) {
		...
		if ((str[i] >= 'A' && str[i] <= 'Z') || (str[i] >= 'a' && str[i] <= 'z') || str[i] == '_') {
			...
			for (int j = 0;
				 str[i] != '\0' && j < size && ((str[i] <= 'z' && str[i] >= 'a') || (str[i] <= 'Z' && str[i] >= 'A') || str[i] == '_' || (str[i] >= '0' && str[i] <= '9'));
				 j++, i++) {
				name_var[j] = str[i];
			}
			for (; str[i] == '(' || str[i] == ')'; i++) {
...
bool syntax_check(const char *string) {
	return true;
}
...
static inline int vlist_set_one(vlist *v, char *name, double val, bool isconst) {
	if (strcmp(v->name, name) == 0) {
		if (v->isconst) {
		  fprintf(stderr,
				  "Error: %s := %lf -> %lf is not possible, \"%s\" is constant.\n", name,
				  v->val, val, name);
		  return -1;
		} else if (isconst){
			fprintf(stderr, "Error %s = %lf => %s := %lf is not possible, \"%s\" have declared as non constant := operation is assigning it to a constant.\n", name, v->val, name, val, name);
			return -1;
		} else {
			v->val = val;
			return 1;
	...
	while (true) {
		...
		if (setvar) {
			...
			for (i = 0; ret[i] != '\0'; i++) {
				...
				if ((ret[i] <= 'Z' && ret[i] >= 'A')||(ret[i] >= 'a' && ret[i] <= 'z') || ret[i] == '_' || (ret[i] >= '0' && ret[i] <= '9')) {
					var_name[i] = ret[i];
				} else {
					syntax_err = true;
					if (ret[i] != '\0' && ret[i+1]== '=') {
			...
			if (!syntax_err) {
				...
				if (!syntax_err){
					if (strcmp(var_name, "quit") == 0 || strcmp(var_name, "exit") == 0 || strcmp(var_name, "dump")== 0) {
